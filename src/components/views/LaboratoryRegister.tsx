...
import { Role, Laboratory, Partner } from '../../types';
...
interface LaboratoryRegisterProps {
  setView: (view: string) => void;
  setUserRole?: (role: Role | null) => void;
  setUserEmail?: (email: string | null) => void;
  onAddLaboratory?: (newLab: Laboratory) => void;
  predefinedPartner?: Partner;
}

export default function LaboratoryRegister({ setView, setUserRole, setUserEmail, onAddLaboratory, predefinedPartner }: LaboratoryRegisterProps) {
  ...
  const [licenseDocument, setLicenseDocument] = React.useState('NABL_Lab_Accreditation.pdf');
  ...
  const handleRegisterSubmit = (e: React.FormEvent) => {
    ...
    if (verifiedPartner) {
      try {
        ...
        if (savedPartners) {
          ...
          const updated = partnersList.map((p: any) => {
            if (p.id === verifiedPartner.id) {
              return {
                ...p,
                onboardedLaboratoriesCount: (p.onboardedLaboratoriesCount || 0) + 1,
                walletBalance: p.walletBalance + 2000 // ₹2,000 commission for onboarding lab
              };
            }
            return p;
          });
        ...
        generateCommission('Subscription', newLabObj.id, `Laboratory: ${newLabObj.name}`, 5000, verifiedPartner.id);
      } catch (err) {
        console.error(err);
      }
    }
    ...
    try {
      logTermsAcceptance(
        newLabObj.email.toLowerCase().trim(),
        newLabObj.name.trim(),
        newLabObj.email.toLowerCase().trim(),
        'laboratory',
        '1.0'
      ...
      console.error('Failed to log laboratory terms acceptance:', logErr);
    }

    if (onAddLaboratory) {
      onAddLaboratory(newLabObj);
    }
    ...
    return (
      <div className="bg-white rounded-xl border border-[#D1E5E5] p-8 text-center max-w-xl mx-auto my-8">
        ...
        <p className="text-xs text-gray-500 mt-2 leading-relaxed">
          The NABL accreditation regulatory files and identity assets for <span className="font-bold text-[#0A6E6E]">{name}</span> have been queued for partner audits.
        </p>
      ...
      <div className="bg-white rounded-xl border border-[#D1E5E5] p-6 shadow-sm">
        ...
        <div className="flex items-center gap-3 border-b border-gray-100 pb-4 mb-6">
          ...
          <div>
            ...
            <h2 className="text-base font-black text-[#1A2B3C] font-heading">Onboard Laboratory Profile</h2>
          </div>
        </div>

        {/* Steps Indicators */}
        <div className="grid grid-cols-4 gap-2 mb-6 text-center border-b border-gray-50 pb-4">
          <div className={`py-1.5 text-[9px] font-black rounded-md ${step >= 1 ? 'bg-[#0A6E6E] text-white' : 'bg-slate-100 text-gray-400'}`}>1. Info & Accreditation</div>
        ...
        <form onSubmit={handleRegisterSubmit} className="space-y-4">
          {step === 1 && (
            <div className="space-y-4">
              <span className="text-xs uppercase font-extrabold text-[#0A6E6E] tracking-wider block">Laboratory & Business Information</span>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-[#1A2B3C] mb-1">Laboratory / Diagnostics Name *</label>
                  <input 
                    type="text" 
                    required 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-[#1A2B3C] mb-1">NABL Accreditation Number *</label>
                  <input 
                    type="text" 
                    required 
                    placeholder="e.g. MC-9810-NABL"
                    value={licenseNumber}
              ...
              <div>
                <label className="block text-xs font-bold text-[#1A2B3C] mb-1">Full Lab / Clinic Address *</label>
                <textarea 
                  required 
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <p className="text-[11px] text-gray-500 leading-relaxed">
                To link your diagnostic laboratory profiles, tests database and accept online appointment orders, an annual hosting fee of ₹5,000 applies.
              </p>
          ...
          {step === 4 && (
            <div className="space-y-4">
              ...
              <div className="border border-dashed border-gray-200 rounded-xl p-4 bg-slate-50">
                <label className="block text-xs font-bold text-slate-800 mb-1">Upload NABL Accreditation Certificate *</label>
                ...
                >
                  <span className="text-[10px] font-bold text-slate-500 block">Click to attach different PDF NABL copy</span>
