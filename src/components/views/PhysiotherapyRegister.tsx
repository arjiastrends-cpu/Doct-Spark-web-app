...
import { Stethoscope, Mail, Phone, Clock, ArrowLeft, Upload, Check, CreditCard, ShieldCheck } from 'lucide-react';
import { Role, Physiotherapy, Partner } from '../../types';
...
interface PhysiotherapyRegisterProps {
  setView: (view: string) => void;
  setUserRole?: (role: Role | null) => void;
  setUserEmail?: (email: string | null) => void;
  onAddPhysiotherapy?: (newPhysio: Physiotherapy) => void;
  predefinedPartner?: Partner;
}

export default function PhysiotherapyRegister({ setView, setUserRole, setUserEmail, onAddPhysiotherapy, predefinedPartner }: PhysiotherapyRegisterProps) {
  const [name, setName] = React.useState(''); // Clinic / Center name
  const [therapistName, setTherapistName] = React.useState(''); // Actual physio doctor name
  ...
  // Licensing Details
  const [registrationNumber, setRegistrationNumber] = React.useState(''); // IAP register number
  const [specialty, setSpecialty] = React.useState('Orthopedic Physiotherapy');
  const [experience, setExperience] = React.useState('5');
  const [licenseDocument, setLicenseDocument] = React.useState('IAP_Registration_Certificate.pdf');
  ...
  const handleRegisterSubmit = (e: React.FormEvent) => {
    ...
    if (step === 1) {
      if (!name || !therapistName || !email || !phone || !registrationNumber || !pincode || !address) {
    ...
    const newPhysioObj: Physiotherapy = {
      id: `physio-${Date.now()}`,
      name: name.trim(),
    ...
    try {
      ...
      localAccounts[email.trim().toLowerCase()] = {
        email: email.trim().toLowerCase(),
        password: password,
        name: therapistName.trim() || name.trim(),
        role: 'physiotherapy'
      };
    ...
    if (verifiedPartner) {
      try {
        ...
        if (savedPartners) {
          ...
          const updated = partnersList.map((p: any) => {
            if (p.id === verifiedPartner.id) {
              return {
                ...p,
                onboardedPhysiotherapistsCount: (p.onboardedPhysiotherapistsCount || 0) + 1,
                walletBalance: p.walletBalance + 2000 // ₹2,000 commission for onboarding physio
              };
            }
            return p;
          });
        ...
        generateCommission('Subscription', newPhysioObj.id, `Physiotherapy: ${newPhysioObj.name}`, 5000, verifiedPartner.id);
      } catch (err) {
        console.error(err);
      }
    }
    ...
    try {
      logTermsAcceptance(
        newPhysioObj.email.toLowerCase().trim(),
        newPhysioObj.name.trim(),
        newPhysioObj.email.toLowerCase().trim(),
        'physiotherapy',
        '1.0'
      );
    } catch (logErr) {
      console.error('Failed to log physiotherapy terms acceptance:', logErr);
    }

    if (onAddPhysiotherapy) {
      onAddPhysiotherapy(newPhysioObj);
    }

    setIsSuccess(true);

    if (setUserRole && setUserEmail) {
      setUserRole('physiotherapy');
      setUserEmail(newPhysioObj.email);
    }
    ...
    return (
      <div className="bg-white rounded-xl border border-[#D1E5E5] p-8 text-center max-w-xl mx-auto my-8">
        ...
        <p className="text-xs text-gray-500 mt-2 leading-relaxed">
          The professional degrees and Indian Association of Physiotherapists regulatory files for <span className="font-bold text-[#0A6E6E]">{therapistName}</span> have been queued for partner audits.
        </p>
      ...
      <div className="bg-white rounded-xl border border-[#D1E5E5] p-6 shadow-sm">
        ...
        <div className="flex items-center gap-3 border-b border-gray-100 pb-4 mb-6">
          ...
          <div>
            ...
            <h2 className="text-base font-black text-[#1A2B3C] font-heading">Onboard Physiotherapist</h2>
          </div>
        </div>

        {/* Steps Indicators */}
        <div className="grid grid-cols-4 gap-2 mb-6 text-center border-b border-gray-50 pb-4">
          <div className={`py-1.5 text-[9px] font-black rounded-md ${step >= 1 ? 'bg-[#0A6E6E] text-white' : 'bg-slate-100 text-gray-400'}`}>1. Info & Experience</div>
        ...
        <form onSubmit={handleRegisterSubmit} className="space-y-4">
          {step === 1 && (
            <div className="space-y-4">
              <span className="text-xs uppercase font-extrabold text-[#0A6E6E] tracking-wider block">Therapist & Business Information</span>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-[#1A2B3C] mb-1">Therapist Name *</label>
                  <input 
                    type="text" 
                    required 
                    placeholder="e.g. Dr. Pooja Sharma (PT)"
                    value={therapistName}
                ...
                <div>
                  <label className="block text-xs font-bold text-[#1A2B3C] mb-1">Clinic / Center Name *</label>
                  <input 
                    type="text" 
                    required 
                    placeholder="e.g. Elite Physio Center"
                    value={name}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-bold text-[#1A2B3C] mb-1">IAP Registration Number *</label>
                  <input 
                    type="text" 
                    required 
                    placeholder="e.g. PT-IAP-9281"
                ...
                <div>
                  <label className="block text-xs font-bold text-[#1A2B3C] mb-1">Physio Specialization *</label>
                  <select
                    value={specialty}
                    onChange={(e) => setSpecialty(e.target.value)}
                    className="w-full bg-white border border-[#D1E5E5] p-2 rounded text-xs font-bold text-slate-700"
                  >
                    <option value="Orthopedic Physiotherapy">Orthopedic Physiotherapy</option>
                    <option value="Sports Physiotherapy">Sports Rehabilitation</option>
                    <option value="Neurological Physiotherapy">Neurological PT</option>
                    <option value="Cardiopulmonary PT">Cardiopulmonary PT</option>
                    <option value="Pediatric Physiotherapy">Pediatric PT</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-bold text-[#1A2B3C] mb-1">Years of Experience *</label>
                  <input 
                    type="number" 
                    required 
                    min={0}
              ...
              <div>
                <label className="block text-xs font-bold text-[#1A2B3C] mb-1">Full Clinic / Consultation Address *</label>
                <textarea 
                  required 
                  placeholder="Street name, building details, cabin/suite details..."
                  value={address}
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <p className="text-[11px] text-gray-500 leading-relaxed">
                To link your professional clinic consultations, receive video physiotherapy requests, and coordinate with DoctSpark healthcare groups, an annual hosting fee of ₹5,000 applies.
              </p>
              ...
              <div className="border border-teal-200 rounded-xl p-4 bg-teal-50/40">
                <div className="flex justify-between items-center text-xs font-extrabold text-[#1A2B3C] mb-2">
                  <span>Physiotherapy Profile Setup</span>
          ...
          {step === 4 && (
            <div className="space-y-4">
              ...
              <div className="border border-dashed border-gray-200 rounded-xl p-4 bg-slate-50">
                <label className="block text-xs font-bold text-slate-800 mb-1">Upload IAP Registration / License *</label>
                ...
                <div 
                  onClick={() => alert('Simulated professional registration file re-upload.')}
                ...
                >
                  <span className="text-[10px] font-bold text-slate-500 block">Click to attach different registration certificate PDF</span>
